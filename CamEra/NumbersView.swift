...
struct AnimatedYearView: View {
    @State private var year: Int = 2010
    let minYear = 2010
    let maxYear = 2015
    ...
    var body: some View {
        VStack {
            if isVisible {
                let yearText = Text(formatYear(year))
    ...
    private func startYearAnimation() {
...
struct TextTransition: Transition {
    ...
    func body(content: Content, phase: TransitionPhase) -> some View {
        ...
        let renderer = AppearanceEffectRenderer(elapsedTime: elapsedTime, totalDuration: duration)
